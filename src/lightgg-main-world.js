...
function extractGradesFromJson(data) {
    ...
    if (Array.isArray(data)) {
        for (const item of data) {
            ...
            const id = String(item.instanceId || item.instance_id || item.id || item.itemInstanceId || '');
...
function dispatchGrades(grades, source) {
    ...
    document.dispatchEvent(new CustomEvent('__aegis_lgg_grades__', {
...
function isLightGGApiUrl(url) {
    try {
        ...
        if (!u.hostname.includes('light.gg'))
            return false;
        // Skip static assets and CDN
        if (/\.(js|css|png|jpg|svg|ico|woff|woff2)(\?|$)/i.test(u.pathname))
            return false;
