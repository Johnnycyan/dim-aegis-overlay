...
let manifestDbName = null;
let itemStoreName = null;
let plugSetStoreName = null;
const weaponNameToHash = {};
// Promise to handle startup race conditions between indexing and content requests
let indexReadyResolve = null;
const indexReadyPromise = new Promise((resolve) => {
...
async function getManifestDictionaryByKey(keyName) {
    if (cachedDictionaries[keyName])
        return cachedDictionaries[keyName];
    if (!manifestDbName)
        return null;
 ...
 */
async function getDefinitionByHash(hash, storeName) {
 ...
 */
async function getWeaponPossiblePerksByName(weaponName) {
    ...
    const hash = weaponNameToHash[normName];
    if (!hash) {
    ...
    const possibleMags = [];
    const possiblePerk1s = [];
    const possiblePerk2s = [];
    ...
    for (const entry of weaponDef.sockets.socketEntries) {
        ...
        const plugSetHashes = [...new Set([
            ...
            ].filter((hash) => typeof hash === 'number' && hash > 0))];
        const plugSets = [];
        let categoryDef = null;
        ...
        if (!categoryDef || plugSets.length === 0)
            continue;
        ...
        for (const plugItems of plugSets) {
            for (const plugItem of plugItems) {
                ...
                if (!plugDef?.displayProperties?.name)
                    continue;
                ...
                if (slotCategory === 'barrel') {
                    if (!possibleBarrels.includes(name))
                        possibleBarrels.push(name);
                }
                else if (slotCategory === 'mag') {
                    if (!possibleMags.includes(name))
                        possibleMags.push(name);
                }
                else if (slotCategory === 'perk1') {
                    if (!possiblePerk1s.includes(name))
                        possiblePerk1s.push(name);
                }
                else if (slotCategory === 'perk2') {
                    if (!possiblePerk2s.includes(name))
                        possiblePerk2s.push(name);
                }
                else if (slotCategory === 'origin') {
                    if (!possibleOrigins.includes(name))
                        possibleOrigins.push(name);
                }
            }
        }
        ...
        for (const mutation of mutations) {
            ...
            if (mutation.type === 'attributes' && mutation.attributeName === 'data-request-weapon-perks') {
                ...
                if (requestStr) {
                    ...
                    let weaponNames = [];
                    try {
                        weaponNames = JSON.parse(requestStr);
                    }
...
const detectPlugCategory = (def) => {
    ...
    if (typeName) {
        if (typeName.includes('barrel') || typeName.includes('scope') || typeName.includes('bowstring') ||
            typeName.includes('sword blade') || typeName.includes('sword guard'))
            return 'barrel';
        if (typeName.includes('magazine') || typeName.includes('battery') ||
            typeName.includes('arrow') || typeName.includes('blade') || typeName.includes('guard'))
            return 'mag';
        if (typeName.includes('origin'))
            return 'origin';
        if (typeName.includes('intrinsic') || typeName.includes('weapon frame') ||
            typeName === 'exotic weapon')
            return 'intrinsic';
        if (typeName.includes('masterwork') || typeName === 'shader')
            return 'skip';
        // "Weapon Perk" and "Weapon Trait" and similar → trait column
        if (typeName.includes('perk') || typeName.includes('trait'))
            return 'trait';
    }
        ...
        if (item.sockets && item.sockets.allSockets) {
            for (const socket of item.sockets.allSockets) {
                ...
                if (socket.plugOptions) {
                    for (const opt of socket.plugOptions) {
                        if (opt.plugDef && opt.plugDef.hash) {
                            ...
                            const plugName = def.displayProperties?.name || '';
                            if (plugName && slotCategory && slotCategory !== 'skip' && slotCategory !== 'intrinsic') {
                                if (!slotNames.includes(plugName))
                                    slotNames.push(plugName);
                            }
                        }
                    }
                ...
                if (slotNames.length > 0) {
                    if (slotCategory === 'barrel') {
                        slotNames.forEach(n => { if (!possibleBarrels.includes(n))
                            possibleBarrels.push(n); });
                    }
                    else if (slotCategory === 'mag') {
                        slotNames.forEach(n => { if (!possibleMags.includes(n))
                            possibleMags.push(n); });
                    }
                    else if (slotCategory === 'perk1') {
                        slotNames.forEach(n => { if (!possiblePerk1s.includes(n))
                            possiblePerk1s.push(n); });
                    }
                    else if (slotCategory === 'perk2') {
                        slotNames.forEach(n => { if (!possiblePerk2s.includes(n))
                            possiblePerk2s.push(n); });
                    }
                    else if (slotCategory === 'origin') {
                        slotNames.forEach(n => { if (!possibleOrigins.includes(n))
                            possibleOrigins.push(n); });
                    }
                }
            }
        }
    ...
    const tryConnect = async () => {
        try {
            ...
            for (const name of dbNames) {
                if (name.includes('google') || name.includes('chrome'))
                    continue;
...
async function buildWeaponIndex() {
    if (!manifestDbName || !itemStoreName)
        return;
        ...
        if (dict) {
            ...
            for (const [hashStr, item] of Object.entries(itemsTable)) {
                ...
                if (itemObj && itemObj.displayProperties && itemObj.displayProperties.name) {
                    const isWeapon = itemObj.itemType === 3 || // DestinyItemType.Weapon
                    ...
                    if (val && typeof val === 'object') {
                        ...
                        if (!val.displayProperties && Object.keys(val).length > 100) {
                            for (const [hashStr, item] of Object.entries(val)) {
                                const itemObj = item;
                                if (itemObj && itemObj.displayProperties && itemObj.displayProperties.name) {
                                    const isWeapon = itemObj.itemType === 3 ||
                                        (itemObj.itemCategoryHashes && itemObj.itemCategoryHashes.includes(1)) ||
                                        (itemObj.sockets && itemObj.itemTypeDisplayName?.toLowerCase().includes('weapon'));
                                    if (isWeapon) {
                                        const name = itemObj.displayProperties.name.toLowerCase().trim();
